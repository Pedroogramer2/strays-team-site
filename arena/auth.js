...
import { 
    getAuth, 
    GoogleAuthProvider, 
    signInWithPopup, // VOLTAMOS PARA O POPUP (Mais estável)
...
// --- SALVAR SESSÃO NO FIRESTORE E LOCALSTORAGE ---
async function saveUserSession(user) {
    ...
    } catch (error) { console.error("Erro ao salvar user no banco:", error); }
    ...
    // AVISA O SITE QUE LOGOU (Correção do Bug)
    window.dispatchEvent(new Event('auth-change'));

    // Redireciona (opcional se você quiser que ele fique na mesma página)
...
window.handleLogout = () => {
    signOut(auth).then(() => {
        ...
        // AVISA O SITE QUE DESLOGOU (Correção do Bug)
        window.dispatchEvent(new Event('auth-change'));
...
// 5. FERRAMENTAS GLOBAIS (Busca e Listagem)
window.searchUserByNick = async (nickToFind) => {
    try {
        const usersRef = collection(db, "users");
        const q = query(usersRef, where("nick", "==", nickToFind));
...
window.changeUserRole = async (targetUid, newRole) => {
    try {
        ...
        console.error(error);
        alert("Erro ao mudar cargo (verifique permissões).");
    }
}